...
#current state [Chargin | Discharging]
current="$(acpi | grep 'Battery 0' | cut -d ' ' -f 4 | sed 's/%,//')"
#current battery level [0 | 1 | 2 | and so on]
	...
	then
		...
		then
			...
			then
				...
				if [ $current -lt $raff ] && [ $current -gt $raffIntense ] || [ $current -eq $raff ]
					#condition on $current, level moderate
				then
					notify
				fi

				if [ $current -lt $raffIntense ] || [ $current -eq $raffIntense ] && [ $current -gt $raffExtreme ]
					#condition on $current, level sustained
				then
					notifyIntense
				fi

				if [ $current -lt $raffExtreme ] || [ $current -eq $raffExtreme ]
					#condition om $current, level OVERKILL
				then
					notifyExtreme
				fi
			else
